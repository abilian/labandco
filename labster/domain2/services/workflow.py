...
from typing import TYPE_CHECKING, Container, Dict, List
...
#     from labster.domain.models.unites import LABORATOIRE, DEPARTEMENT, EQUIPE
#
#     structure = demande.structure
...
class PrendreLaMainGestionnaire(AbstractPrendreLaMain):
    ...
    def precondition(self, workflow: Workflow):
        ...
        #     case.porteur,
        # )
        return actor.has_role(Role.GESTIONNAIRE, "*") and actor not in (
            case.gestionnaire,
            case.porteur,
        )
        ...
        for history_item in demande.wf_history:
            ...
            if actor_id:
                ...
                if isinstance(actor_id, int):
                    ...
                    actor = db.session.query(Profile).get(actor_id).first()
                if actor:
